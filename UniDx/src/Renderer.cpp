...
// クローン固有状態の初期化
// -----------------------------------------------------------------------------
void Renderer::CloneTo(Component& destination) const
{
    auto& renderer = static_cast<Renderer&>(destination);

    // GPUリソースは複製元と共有せず、OnEnable()で作り直す
    renderer.constantBufferPerObject.Reset();
}
